const domController = (() => {
	...
	const addProjectToSelect = name => {
		const template = document.querySelector('#new-item-template-prompt');
	...
	const fillItemEditModeWithItem = (itemDiv, item) => {
		...
		itemDiv.appendChild(getItemTemplateElements('new-item-template-prompt'));
	...
	const renderNewItemPrompt = () => {
		const itemDiv = document.createElement('div');
		const itemPromptNodes = getItemTemplateElements('new-item-template-prompt');
